...
let simpler_sum xs = List.fold xs ~init:0 ~f:(fun x -> fun y -> x + y);;
let simpler_product xs = List.fold xs ~init:1 ~f:(fun x -> fun y -> x * y);;
...
let float_of_int xs = List.map xs ~f:(fun x -> Float.of_int x);;
...
let print_int_list xs = List.iter xs ~f:(fun s -> Stdio.printf "%d " s);;
