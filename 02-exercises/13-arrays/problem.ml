...
let double array : unit = Array.iteri array ~f:(fun i e -> Array.set array i (e*2));;
...
let double_selectively array indices : unit = List.iter indices ~f:(fun i : unit -> array.(i) <- (array.(i)*2));;
...
let double_matrix matrix : unit = Array.iteri matrix 
    ~f:(fun i _ -> Array.iteri matrix.(i) ~f:(fun j e -> matrix.(i).(j) <- (e*2)))
;;
